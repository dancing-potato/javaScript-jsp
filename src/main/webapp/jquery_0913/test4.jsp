...
<script type="text/javascript">
	...
	- $("선택자").css("속성명", function() {}) 형식으로 구현
		...
		=> 이때, 익명 함수 내의 파라미터 설정에 따라 각각 다른 값이 자동으로 전달됨
	
	1. $("선택자").css("속성명", function(index){})
		- 해당 선택자에 대한 지정된 속성값에 차례대로 반복 접근
		- 이때, 익명 함수 파라미터인 index에 해당 요소의 인덱스가 자동으로 전달됨
	
		
	2. $("선택자").css("속성명", function(index,value){})
		- 해당 선택자에 대한 지정된 속성값에 차례대로 반복 접근
		- 이때, 익명함수 파라미터인 index에 해당 요소의 인덱스가 자동으로 전달되고,
...
// 		alert(h3Color);		// rgb(0,0,0) 출력됨
		...
		// 첫번째 h3태그의 color 속성값을 blue로 변경 후 색상값 출력
		$("h3:first").css("color","blue");
// 		alert($("h3:first").css("color"));		// rgb(0,0,255) 출력됨
		
// ==================================================================================
		// 복수개의 h3 태그의 차례대로 접근하여 해당 요소의 인덱스 가져와서 출력
...
// 			// 첫번째 파라미터(index) : 해당 요소의 인덱스(0부터 시작) 차례대로 전달(=1씩증가 = 0 1 2)
// 			// 두번째 파라미터(value) : 인덱스에 해당하는 요소의 지정된 속성값 차례대로 전달(BLUE, BLACK, RED의 RGB값)
// 			alert("h3 태그 인덱스 : " + index + "\nh3태그 color 속성값 : " + value);
// 		});
		
	// -----------------------------------------------------------------------------
	// 익명 함수를 구현한 css() 함수 활용법
	// 자바스크립트에서 배열만들때는 대괄호[]만 주면 된다
	var colorValue = ["GREEN","MAGENTA","SKYBLUE"];		 // 글자색을 저장하는 배열 생성
	var backgroundColorValue = ["YELLOW","BLACK","RED"]; // 배경색을 저장하는 배열 생성	
		
	// h3 태그의 css함수를 호출하여 color 속성에 차례대로 접근(파라미터로 인덱스값 지정)
// 	$("h3").css("color", function(index){
		// 익명 함수 내에서 return 문을 사용하여 특정 값 리턴 시 해당 요소의 속성값 변경가능
...
// 		});
	...
	// h3태그의 css() 함수를 호출하여 background 속성에 차례대로 접근 후
